 ...
 */
type FilterArrayByValue<T extends unknown[] | undefined, C, Acc extends unknown[] = []> = T extends [infer Head, ...infer Tail] ? Head extends C ? FilterArrayByValue<Tail, C, [...Acc, Head]> : FilterArrayByValue<Tail, C, Acc> : Acc;
/**
 ...
 * @details - this is using tail recursion type optimization from typescript 4.5
 */
type FilterArrayByKey<T extends unknown[], K extends string, Acc extends unknown[] = []> = T extends [infer Head, ...infer Tail] ? Head extends {
    [Key in K]: unknown;
} ? FilterArrayByKey<Tail, K, [...Acc, Head]> : FilterArrayByKey<Tail, K, Acc> : Acc;
/**
 * filter an array type by removing undefined values
 * @param T - array type
 * @details - this is using tail recursion type optimization from typescript 4.5
 */
type DefinedArray<T extends unknown[], Acc extends unknown[] = []> = T extends [infer Head, ...infer Tail] ? Head extends undefined ? DefinedArray<Tail, Acc> : DefinedArray<Tail, [Head, ...Acc]> : Acc;
 ...
 * @param T - union type
 */
type MergeUnion<T> = (T extends unknown ? (k: T) => void : never) extends (k: infer I) => void ? {
    [K in keyof I]: I[K];
} : never;
/**
 ...
 * @param T - object type
 */
type UndefinedToOptional<T> = Merge<RequiredProps<T>, Partial<OptionalProps<T>>>;
/**
 ...
 */
type NeverIfEmpty<T> = IfEquals<T, {}, never, T>;
/**
 ...
 * type B = NotEmpty<A>; // B = never
 */
type UndefinedIfEmpty<T> = IfEquals<T, {}, undefined, T>;
 ...
 * @param T - object type
 */
type SetPropsOptionalIfChildrenAreOptional<T> = Merge<Pick<Partial<T>, OptionalChildProps<T>>, Pick<T, RequiredChildProps<T>>>;
/**
 ...
 * @param T - type
 */
type ReadonlyDeep<T> = T extends (infer R)[] ? ReadonlyArrayDeep<R> : T extends Function ? T : T extends object ? DeepReadonlyObject<T> : T;
type MaybeReadonly<T> = T | ReadonlyDeep<T>;
/**
 * Map a type an api description parameter to a zod infer type
 * @param T - array of api description parameters
 * @details -  this is using tail recursion type optimization from typescript 4.5
 */
type MapSchemaParameters<T, Frontend extends boolean = true, Acc = {}> = T extends [infer Head, ...infer Tail] ? Head extends {
    name: infer Name;
    schema: infer Schema;
} ? Name extends string ? MapSchemaParameters<Tail, Frontend, Merge<{
 ...
 * @details - this is using tail recursion type optimization from typescript 4.5
 */
type PathParamNames<Path, Acc = never> = Path extends `${string}:${infer Name}/${infer R}` ? PathParamNames<R, Name | Acc> : Path extends `${string}:${infer Name}` ? Name | Acc : Acc;
/**
 ...
 */
type Assert<T, U> = IfEquals<T, U, true, {
    error: "Types are not equal";
    type1: T;
    type2: U;
}>;
type PickRequired<T, K extends keyof T> = Merge<T, {
    [P in K]-?: T[P];
}>;
/**
 ...
 * ```
 */
type TupleFlat<T, Acc extends unknown[] = []> = T extends [
    infer Head,
    ...infer Tail
] ? Head extends unknown[] ? TupleFlat<Tail, [...Acc, ...Head]> : TupleFlat<Tail, [...Acc, Head]> : Acc;
/**
...
export { Assert, DeepReadonlyObject, DefinedArray, FilterArrayByKey, FilterArrayByValue, IfEquals, MapSchemaParameters, MaybeReadonly, Merge, MergeUnion, Narrow, NeverIfEmpty, OptionalChildProps, OptionalProps, PathParamNames, PickDefined, PickRequired, ReadonlyDeep, RequiredKeys, RequiredProps, SetPropsOptionalIfChildrenAreOptional, Simplify, TupleFlat, UndefinedIfEmpty, UndefinedIfNever, UndefinedToOptional, UnionToIntersection, UnionToTuple };
